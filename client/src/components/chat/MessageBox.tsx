...
interface MessageBoxProps {
  ...
  onSend?: () => void;
  onFileSend?: (files: File[], message?: string) => void;
  chatId: string;
  isTyping?: boolean;
}
...
const MessageBox: React.FC<MessageBoxProps> = ({
  ...
  return (
    <div className="flex flex-col w-full max-w-8xl py-3">
      {/* File Preview */}
      {selectedFile && (
      ...
      <div className="flex border-2 border-gray-700 bg-gray-900 rounded-full items-center px-3 py-2 gap-3">
        ...
        {/* Conditional rendering: Voice or Send */}
        {!message.trim() && !selectedFile && !isTyping ? (
          ...
          <button
            ...
            className="w-7 h-7 text-black rounded-full bg-green-500 p-1 cursor-pointer hover:bg-green-600 transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
